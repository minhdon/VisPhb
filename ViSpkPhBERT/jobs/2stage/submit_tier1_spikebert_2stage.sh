...
SEED="${SEED:-42}"
RUN_NAME="tier1_spikebert_2stage"
ENTRYPOINT="tier1_spikebert_2stage.py"

SCRIPT_PATH="$PROJECT_ROOT/uit-train/entrypoints/$ENTRYPOINT"

SPIKING_LAYERS="${SPIKING_LAYERS:-4}"
SPIKE_TIMESTEPS="${SPIKE_TIMESTEPS:-16}"

STAGE1_CKPT="${STAGE1_CKPT:-$PROJECT_ROOT/uit-models/stage1_wiki_bptt/spk${SPIKING_LAYERS}_T${SPIKE_TIMESTEPS}/seed_${SEED}/best_stage1_wiki_bptt_seed${SEED}.pt}"
...
OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT_ROOT/uit-models/$RUN_NAME/spk${SPIKING_LAYERS}_T${SPIKE_TIMESTEPS}/seed_${SEED}}"
LOG_DIR="$PROJECT_ROOT/logs"
CACHE_ROOT="$DATASTORE_ROOT/hf_cache/$RUN_NAME/spk${SPIKING_LAYERS}_T${SPIKE_TIMESTEPS}/seed_${SEED}/${SLURM_JOB_ID:-manual}"
...
RESET_MODE="${RESET_MODE:-subtract}"
...
echo "[CONFIG] SEED=$SEED SPIKING_LAYERS=$SPIKING_LAYERS SPIKE_TIMESTEPS=$SPIKE_TIMESTEPS TASK_KD_EPOCHS=$TASK_KD_EPOCHS PREDICTION_KD_EPOCHS=$PREDICTION_KD_EPOCHS"
...
python -u "$SCRIPT_PATH" \
    ...
    --spike_timesteps "$SPIKE_TIMESTEPS" \
    --beta "$BETA" \
    --threshold "$THRESHOLD" \
