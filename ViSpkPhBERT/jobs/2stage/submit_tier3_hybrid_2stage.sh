...
SEED="${SEED:-42}"
RUN_NAME="tier3_hybrid_2stage"
ENTRYPOINT="tier3_hybrid_2stage.py"
...
LAMBDA_EMBEDDING="${LAMBDA_EMBEDDING:-0.05}"

ALPHA_CE_FINAL="${ALPHA_CE_FINAL:-0.5}"
LAMBDA_F_FINAL="${LAMBDA_F_FINAL:-0.0}"
LAMBDA_EMBEDDING_FINAL="${LAMBDA_EMBEDDING_FINAL:-0.0}"

CURRICULUM_WARMUP_RATIO="${CURRICULUM_WARMUP_RATIO:-0.02}"
CURRICULUM_WARMUP_STEPS="${CURRICULUM_WARMUP_STEPS:-0}"
CURRICULUM_MIN_TASK_WEIGHT="${CURRICULUM_MIN_TASK_WEIGHT:-0.2}"
...
python -u "$SCRIPT_PATH" \
    ...
    --lambda_embedding_final "$LAMBDA_EMBEDDING_FINAL" \
    --curriculum_warmup_ratio "$CURRICULUM_WARMUP_RATIO" \
    --curriculum_warmup_steps "$CURRICULUM_WARMUP_STEPS" \
    --curriculum_min_task_weight "$CURRICULUM_MIN_TASK_WEIGHT" \
