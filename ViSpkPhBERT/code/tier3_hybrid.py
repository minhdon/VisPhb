...
Standalone hybrid-curriculum implicit SNN knowledge distillation script for
...
Tier 3 adds:
    - embedding alignment loss L_emb
    - curriculum schedule based on global_step:
        warmup: L_feat + lambda_e * L_emb
        main:   alpha * L_CE + (1-alpha) * L_KD + lambda_f * L_feat + lambda_e * L_emb
...
LOGGER = logging.getLogger("tier3_hybrid")
...
class Config:
    ...
    warmup_curriculum_ratio: float = 0.1

    output_dir: str = "outputs/tier3"
...
class ImplicitSNNFFN(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # Anderson mixing can produce non-binary z_star. Firing rate should be spike fraction.
...
class ImplicitPhoBERTStudent(nn.Module):
    ...
    def forward(
        ...
        need_hidden_states = output_hidden_states or return_embeddings
        ...
        if return_embeddings:
            if outputs.hidden_states is None:
                raise RuntimeError(
                    "return_embeddings=True requires encoder hidden_states, "
                    "but outputs.hidden_states is None."
                )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tier 3 hybrid-curriculum implicit PhoBERT to SNN KD for UIT-VSFC"
    )
    ...
    parser.add_argument("--warmup_curriculum_ratio", type=float, default=0.1)

    parser.add_argument("--output_dir", type=str, default="outputs/tier3")
    ...
    parser.add_argument("--lambda_e", type=float, default=0.05)
...
def compute_embedding_loss(
    ...
    projected_student_embedding = student.embedding_projector(student_embedding.float())
    return F.mse_loss(projected_student_embedding, teacher_embedding.detach().float())
            ...
            student_layers.append(
                {
                    ...
                    "note": (
                        "Tier 3 uses implicit fixed-point inference. "
...
def train_one_epoch(
    ...
    steps = 0
    warmup_steps_this_epoch = 0
    main_steps_this_epoch = 0
    phase_at_end = "warmup" if global_step < curriculum_warmup_steps else "main"
    ...
    for batch in loader:
        ...
        if global_step == curriculum_warmup_steps:
            print(f"[CURRICULUM] Switching to main phase at step {global_step}", flush=True)

        phase = "warmup" if global_step < curriculum_warmup_steps else "main"
        phase_at_end = phase

        if phase == "warmup":
            warmup_steps_this_epoch += 1
        else:
            main_steps_this_epoch += 1
        ...
        with torch.no_grad():
            ...
            teacher_embedding = teacher_outputs["hidden_states"][0]
        ...
        with amp_autocast(device, use_amp):
            ...
            student_embedding = student_outputs["embeddings"]
            ...
            if phase == "warmup":
                ce_loss = torch.zeros((), dtype=torch.float32, device=device)
                kd_loss = torch.zeros((), dtype=torch.float32, device=device)
                loss = feature_loss + config.lambda_e * embedding_loss
            else:
                ce_loss = ce_loss_fn(
                ...
                loss = (
                    ...
                    + config.lambda_e * embedding_loss
                )
        ...
        total_emb += float(embedding_loss.detach().item())
    ...
    return {
        ...
        "embedding": total_emb / denom,
        "firing_rate": student.mean_firing_rate(),
        "epoch": float(epoch),
        "warmup_steps": float(warmup_steps_this_epoch),
        "main_steps": float(main_steps_this_epoch),
...
def build_config(args: argparse.Namespace) -> Config:
    return Config(
        ...
        warmup_curriculum_ratio=args.warmup_curriculum_ratio,
...
def main(args: argparse.Namespace) -> None:
    ...
    best_checkpoint = output_dir / f"best_tier3_seed{config.seed}.pt"
    ...
    lr_warmup_steps = int(config.warmup_ratio * total_steps)
    curriculum_warmup_steps = int(config.warmup_curriculum_ratio * total_steps)

    LOGGER.info(
        "Total steps=%d | LR warmup steps=%d | Curriculum warmup steps=%d",
        total_steps,
        lr_warmup_steps,
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        LOGGER.info(
            "Epoch %d/%d | train_loss=%.4f ce=%.4f kd=%.4f feat=%.4f emb=%.4f | "
        ...
        print(
            ...
            f"warmup_steps={int(train_stats['warmup_steps'])} "
            f"main_steps={int(train_stats['main_steps'])} "
            f"phase_at_end={train_stats['phase_at_end']}",
            flush=True,
        )

        print(
            f"[MARKER] tier=tier3 seed={config.seed} epoch={epoch} "
    ...
    print(
        f"[MARKER] tier=tier3 seed={config.seed} TEST "
    ...
    print(
        f"[MARKER] convergence tier=tier3 "
    ...
    print(
        f"[MARKER] theoretical_energy tier=tier3 "
