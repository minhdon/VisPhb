...
Tier 1 full two-stage BPTT spiking PhoBERT training.

Pipeline:
    Stage 1:
        Already trained by stage1_wiki_bptt_tier1.py
        PhoBERT pretrained teacher -> SpikeBERTStudent BPTT on segmented Wiki.

    Stage 2A:
        Task-based KD on UIT-VSFC:
...
This file is intended to be a Vietnamese SpikeBERT-inspired full two-stage
BPTT PhoBERT-SNN KD pipeline.

Important:
    This is not an absolute reproduction of SpikeBERT because the student is a
    hybrid PhoBERT model with selected FFN blocks replaced by BPTT spiking FFN,
    while self-attention remains ANN.
"""
...
LOGGER = logging.getLogger("tier1_spikebert_2stage")
...
# =========================================================
# Import Stage-1 BPTT module
# =========================================================

def import_stage1_bptt_module():
    module_name = "stage1_wiki_bptt_runtime"

    existing = sys.modules.get(module_name)
    if existing is not None and hasattr(existing, "Config") and hasattr(existing, "SpikeBERTStudent"):
        return existing
    ...
    module_path = script_dir / "stage1_wiki_bptt_tier1.py"

    if not module_path.exists():
        raise FileNotFoundError(
            f"Cannot find stage1_wiki_bptt_tier1.py at {module_path}. "
            f"Put tier1_spikebert_2stage.py and stage1_wiki_bptt_tier1.py "
            f"in the same folder."
        )

    spec = importlib.util.spec_from_file_location(module_name, str(module_path))
    if spec is None or spec.loader is None:
        raise ImportError(f"Cannot import stage1_wiki_bptt_tier1.py from {module_path}")
...
_STAGE1_BPTT = import_stage1_bptt_module()

Stage1BPTTConfig = _STAGE1_BPTT.Config
SpikeBERTStudent = _STAGE1_BPTT.SpikeBERTStudent
safe_torch_load = _STAGE1_BPTT.safe_torch_load
amp_autocast = _STAGE1_BPTT.amp_autocast
make_grad_scaler = _STAGE1_BPTT.make_grad_scaler
get_peak_memory_report = _STAGE1_BPTT.get_peak_memory_report
...
class Config:
    ...
    output_dir: str = "uit-models/tier1_spikebert_2stage"
...
def make_default_stage1_ckpt(project_root: Path, config: Config) -> Path:
    return (
        ...
        / "uit-models"
        / "stage1_wiki_bptt"
        / f"spk{int(config.spiking_layers)}_T{int(config.spike_timesteps)}"
        / f"seed_{int(config.seed)}"
        / f"best_stage1_wiki_bptt_seed{int(config.seed)}.pt"
    )
...
def validate_stage1_checkpoint_config(checkpoint: Any, config: Config, ckpt_path: Path) -> None:
    ...
    for key, expected_value in expected.items():
        raw_value = get_checkpoint_config_value(checkpoint, key)

        if raw_value is None:
            LOGGER.warning(
                "[STAGE1 BPTT LOAD] checkpoint=%s has no config.%s metadata; "
        ...
        if actual_value != expected_value:
            mismatches.append(f"{key}: checkpoint={actual_value}, requested={expected_value}")

    if mismatches:
        raise ValueError(
            "Stage-1 checkpoint architecture does not match requested Tier-1 config. "
            f"checkpoint={ckpt_path} mismatches={mismatches}"
        )


def build_stage1_config(config: Config) -> Stage1BPTTConfig:
    stage_cfg = Stage1BPTTConfig()
    ...
    stage_cfg.spike_timesteps = config.spike_timesteps
    stage_cfg.beta = config.beta
    stage_cfg.threshold = config.threshold
    stage_cfg.surrogate_alpha = config.surrogate_alpha
    stage_cfg.reset_mode = config.reset_mode
...
def load_student(config: Config, device: torch.device, project_root: Path):
    ...
    student = SpikeBERTStudent(stage_cfg)
    ...
    if stage1_ckpt.exists():
        ...
        LOGGER.info("[STAGE1 BPTT LOAD] checkpoint=%s", stage1_ckpt)
        LOGGER.info("[STAGE1 BPTT LOAD] strict=%s", config.load_stage1_strict)
        LOGGER.info("[STAGE1 BPTT LOAD] missing_keys=%s", incompatible.missing_keys)
        LOGGER.info("[STAGE1 BPTT LOAD] unexpected_keys=%s", incompatible.unexpected_keys)

        if len(incompatible.missing_keys) > 30 or len(incompatible.unexpected_keys) > 30:
            LOGGER.warning(
                "[STAGE1 BPTT LOAD] Many missing/unexpected keys. "
                "Check spiking_layers, spike_timesteps, reset_mode, and architecture compatibility."
            )
    else:
        raise FileNotFoundError(f"Stage-1 BPTT checkpoint not found: {stage1_ckpt}")
...
def compute_loss(
    ...
    if phase in ("task_kd", "task_ikd"):
        ...
        raise ValueError(
            f"Unknown phase: {phase}. Expected one of: task_ikd, task_kd, prediction_kd."
        )
...
def compute_theoretical_energy_report(student, config: Config) -> Dict[str, Any]:
    ...
    Assumptions:
        ...
        - Spiking FFN layers use AC cost scaled by:
              spike_timesteps * measured firing_rate * equivalent FFN FLOPs.
        - This follows the common theoretical neuromorphic estimate, not GPU energy.
    """
    mac_pj = 4.6
    ac_pj = 0.9
    ...
    spiking_indices = set(int(x) for x in getattr(student, "spiking_layer_indices", []))
    ...
    for layer_idx in range(num_layers):
        ...
        if layer_idx in spiking_indices:
            ...
            timesteps = float(stat.get("spike_timesteps", config.spike_timesteps))
            ffn_energy = timesteps * firing_rate * ffn_flops * ac_pj

            layers.append(
                {
                    "name": f"layer_{layer_idx}_ffn_BPTT_SNN",
                    "type": "SNN_FFN_AC",
                    "flops_equiv": float(ffn_flops),
                    "spike_timesteps": float(timesteps),
    ...
    return {
        "note": (
            ...
            "Only BPTT spiking FFN layers use AC cost scaled by measured firing rate and spike_timesteps. "
            "This is not actual GPU energy."
        ),
        "mac_pj": mac_pj,
        "ac_pj": ac_pj,
        "seq_len": seq_len,
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        if config.log_every_steps > 0 and global_step % config.log_every_steps == 0:
            print(
                f"[MARKER] tier=tier1_2stage seed={config.seed} "
...
def train_all(
    ...
    best_checkpoint = output_dir / f"best_tier1_spikebert_2stage_seed{config.seed}.pt"
    last_checkpoint = output_dir / f"last_tier1_spikebert_2stage_seed{config.seed}.pt"
    ...
    for epoch in range(start_epoch + 1, total_epochs + 1):
        ...
        if config.save_every_steps > 0 and global_step % config.save_every_steps == 0:
            periodic_path = output_dir / f"periodic_tier1_spikebert_2stage_seed{config.seed}_step{global_step}.pt"
            save_checkpoint(
                path=periodic_path,
                student=student,
                optimizer=optimizer,
    ...
    report = {
        ...
        "note": (
            "Tier 1 full two-stage BPTT PhoBERT-SNN KD. "
            "Stage 1 checkpoint is loaded from stage1_wiki_bptt_tier1.py. "
            "Stage 2 uses task KD followed by prediction-layer distillation."
        ),
    }
    ...
    report_path = output_dir / f"tier1_spikebert_2stage_report_seed{config.seed}.json"
    ...
    print(
        f"[MARKER] tier=tier1_2stage DONE seed={config.seed} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Tier 1 full two-stage BPTT SpikeBERT-style PhoBERT-SNN KD."
    )
    ...
    parser.add_argument("--output_dir", type=str, default="uit-models/tier1_spikebert_2stage")
